...
adminsFilePath = __dirname + '\\admins.json';
usersFilePath = __dirname + '\\users.json';
coursesFilePath = __dirname + '\\courses.json';

try{
  ADMINS = JSON.parse(fs.readFileSync(adminsFilePath,'utf8'));
  USERS = JSON.parse(fs.readFileSync(usersFilePath,'utf8'));
  COURSES = JSON.parse(fs.readFileSync(coursesFilePath,'utf8'));
}
catch{
  ADMINS = [];
  USERS = [];
...
app.post('/admin/signup', (req, res) => {
  ...
  if (userAlreadyPresent) {
    ...
    ADMINS.push(admin);
    fs.writeFileSync(adminsFilePath, JSON.stringify(ADMINS), 'utf8');
...
app.post('/admin/courses', authenticateJwt, (req, res) => {
  ...
  if(course){
    ...
    COURSES.push(course)
    fs.writeFileSync(coursesFilePath, JSON.stringify(COURSES), 'utf8');
